...
用法:
  apitester judge <yaml_path> [--judge-url <url>] [--judge-key <key>] [--judge-model <model>]
"""
...
async def run_judge(yaml_path: str, judge_url: str, judge_key: str, judge_model: str):
    """执行评判"""
    if not judge_key:
    ...
    print(f"📊 正在分析: {yaml_path}")
    print(f"   Judge: {judge_model} @ {judge_url}")

    judgment = await judge(data, judge_url, judge_key, judge_model)
...
def main():
    ...
    import asyncio
    asyncio.run(run_judge(args.yaml_path, args.judge_url, args.judge_key, args.judge_model))
